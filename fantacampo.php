...
if (isset($test)) error_reporting(E_ALL);
...
for ($num1 = 0 ; $num1 <= $num_titolari; $num1++) {
	...
	if     ($numero_titolare >  100 and $numero_titolare <= 200)
	{ if ($cp == 0) {$cp++; $titolari_1[]=$numero_titolare;}
	elseif (isset($test)) echo "C'e' piu' di un portiere! <b>($numero_titolare)</b> <br />";
}
elseif ($numero_titolare >= 201 and $numero_titolare <= 500) { $cd++; $titolari_1[]=$numero_titolare;}
elseif ($numero_titolare >= 501 and $numero_titolare <= 800) { $cc++; $titolari_1[]=$numero_titolare;}
elseif ($numero_titolare >= 801 and $numero_titolare <= 999) { $ca++; $titolari_1[]=$numero_titolare;}
} # fine for $num1
unset($num1);
...
for ($num1 = 0 ; $num1 <= $num_titolari; $num1++) {
	...
	if ($riga_dati_calciatori != -1 ) {
		...
		if ($mnumero == $numero_titolare and $contatore <= 11 ) {
			...
			if ($orientamento_campetto == 0) {
				...
				$spaziatura = 2;
				$altezza_testo = $altezza+$alt_imm2+$spaziatura-4;
				$larghezza1 = $larghezza-strlen($mnome)+20;
				ImageString($campo,2,$larghezza1,$altezza_testo,$mnome,$colore_testo);
			...
			elseif ($orientamento_campetto == 1) {
				...
				$spaziatura = 2;
				$altezza_testo = $altezza+$alt_imm2+$spaziatura-4;
				$larghezza1 = $larghezza-strlen($mnome1)+20;
				ImageString($campo,2,$larghezza1,$altezza_testo,$mnome,$colore_testo);
				#ImageString($campo,1,$larghezza1,$altezza_testo+8,$mnome1,$colore_testo);
				$contatore++; $larghezza=0;
	...
	elseif ($riga_dati_calciatori == -1 and $contatore <= 11 ) {
		...
		if ($orientamento_campetto == 0) {
			...
			elseif ($numero_titolare >= 201 and $numero_titolare <= 500) {
				$larghezza = $lar_imm1-($lar_imm2*$cd); $larghezza = $larghezza/($cd+1); $larghezza=($larghezza+$lar_imm2)*$spazio_d-$lar_imm2;
				$altezza = ($alt_imm1-$alt_imm2)/4+$alt_p;
				$spazio_d++;
			}
			elseif ($numero_titolare >= 501 and $numero_titolare <= 800) {
				$larghezza = $lar_imm1-($lar_imm2*$cc); $larghezza = $larghezza/($cc+1); $larghezza=($larghezza+$lar_imm2)*$spazio_c-$lar_imm2;
				$altezza = ($alt_imm1-$alt_imm2)*2/4+$alt_p;
				$spazio_c++;
			}
			elseif ($numero_titolare >= 801 and $numero_titolare <= 999) {
				$larghezza = $lar_imm1-($lar_imm2*$ca); $larghezza = $larghezza/($ca+1);  $larghezza=($larghezza+$lar_imm2)*$spazio_a-$lar_imm2;
				$altezza = ($alt_imm1-$alt_imm2)*3/4+$alt_p;
				$spazio_a++;
			}
			...
			$spaziatura = 2;
			$altezza_testo = $altezza+$alt_imm2+$spaziatura-4;
			$larghezza1 = $larghezza-strlen(" ".$titolari[$contatore-1])+20;
			ImageString($campo,2,$larghezza1,$altezza_testo," ".$titolari[$contatore-1],$colore_testo);
			//ImageString($campo,1,$larghezza1,$altezza_testo+8,$mnome1,$colore_testo);
			$contatore++; $larghezza=0;
		}
		elseif (@$orientamento_campetto == 1) {
...
$riga_testo=5;
ImageString($campo,2,10,$riga_testo,"Formazione attuale",$colore_testo);
$riga_testo+=10;
ImageString($campo,2,10,$riga_testo,"Modulo: $cd-$cc-$ca",$colore_testo);

if ( (  isset($_GET['iutente']) and  isset($_GET['id_gioc']) and $iutente!="" ) or
( !isset($_GET['iutente']) and !isset($_GET['id_gioc']) and $iutente!="" )
) {$riga_testo+=10; ImageString($campo,2,10,$riga_testo,"Utente: $iutente",$colore_testo); }

if ($nome_squadra != ""  )  { $riga_testo+=10; ImageString($campo,2,10,$riga_testo,"Squadra: $nome_squadra",$colore_testo); }
#if ($ultima_giornata != "") { $riga_testo+=10; ImageString($campo,2,10,$riga_testo,"Giornata: $ultima_giornata",$colore_testo); }
