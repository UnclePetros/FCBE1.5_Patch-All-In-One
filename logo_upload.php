...
if(isset($_FILES['fileup']) && strlen($_FILES['fileup']['name']) > 1) {
  ...
  $uploadpathOrig = $uploadpath.$_SESSION['utente'].".jpg";    // gets the file name
  ...
  if($err == '') {
    ...
    if(move_uploaded_file($_FILES['fileup']['tmp_name'], $uploadpathOrig)) { 
      ...
      $uploadpathMin = $uploadpath.$_SESSION['utente']."_min.png";    // gets the file name
      
      $width_min=90;
      
      // Fix Width & Heigh (Auto calculate)
      
      $height_min=round($width_min*$height/$width);
      $images_orig = imagecreatefromstring(file_get_contents($uploadpathOrig));
      $photoX = ImagesX($images_orig);
      $photoY = ImagesY($images_orig);
      $images_fin = ImageCreateTrueColor($width_min, $height_min);
      ...
      ImageCopyResampled($images_fin, $images_orig, 0, 0, 0, 0, $width_min+1, $height_min+1, $photoX, $photoY);
      imagepng($images_fin,$uploadpathMin);
