...
if ($_SESSION['valido'] == "SI" and $_SESSION['permessi'] >= 4) {
	...
	for($num = 1 ; $num < $num_tornei; $num++) {
		...
		@list($otid, $otdenom, $otpart, $otserie, $otmercato_libero, $ottipo_calcolo, $otgiornate_totali, $otritardo_torneo, $otcrediti_iniziali, $otnumcalciatori, $otcomposizione_squadra, $temp1, $temp2, $temp3, $temp4, $otstato, $otmodificatore_difesa, $otschemi, $otmax_in_panchina, $otpanchina_fissa, $otmax_entrate_dalla_panchina, $otsostituisci_per_ruolo, $otsostituisci_per_schema,  $otsostituisci_fantasisti_come_centrocampisti, $otnumero_cambi_max, $otrip_cambi_numero, $otrip_cambi_giornate, $otrip_cambi_durata, $otaspetta_giorni, $otaspetta_ore, $otaspetta_minuti, $otnum_calciatori_scambiabili, $otscambio_con_soldi, $otvendi_costo, $otpercentuale_vendita, $otsoglia_voti_primo_gol, $otincremento_voti_gol_successivi, $otvoti_bonus_in_casa, $otpunti_partita_vinta, $otpunti_partita_pareggiata, $otpunti_partita_persa, $otdifferenza_punti_a_parita_gol, $otdifferenza_punti_zero_a_zero, $otdifferenza_punti_prima_soglia_meno_sei, $otdifferenza_punti_gol_premio, $otmin_num_titolari_in_formazione, $otpunti_pareggio, $otpunti_pos, $otreset_scadenza) = explode(",", $tornei[$num]);
		...
		$differenza_punti_prima_soglia_meno_sei = $otdifferenza_punti_prima_soglia_meno_sei; 	#	a parità di gol, se una sola delle due squadre ha totalizzato un punteggio inferiore a (soglia primo gol - 6), e se il distacco punti tra le due squadre è inferiore a quello impostato, prende un ulteriore gol. Impostare a zero per disabilitare
		$differenza_punti_gol_premio = $otdifferenza_punti_gol_premio; 				#	se il distacco punti tra le due squadre è superiore a quello impostato, la squadra col punteggio maggiore incrementa di 1 il numero di gol. Impostare a zero per disabilitare
		...
		$espulsione = array();
		$autogol = array();
		$rigorisbagliati = array();
		$golsubiti = array();
		
		# variabili per classifica di giornata
		$punti_tot = array();
		$giocate = array();
		$vinte = array();
		$nulle = array();
		...
		$nulle_casa = array();
		$perse_casa = array();
		$reti_fatte_casa = array();
		$reti_subite_casa = array();
		$giocate_fuori = array();
		$vinte_fuori = array();
		$nulle_fuori = array();
		$perse_fuori = array();
		$reti_fatte_fuori = array();
		$reti_subite_fuori = array();
		$reti_fatte = array();
		$reti_subite = array();
		$differenza_reti = array();
		$totale_punti_fatti = array();
		#Fine variabili per classifica di giornata
		...
		for($num1 = 0 ; $num1 < $num_voti ; $num1++) {
			...
			#bonus
			$golsegnati[$num_calciatore] = $dati_voto[($ncs_golsegnati-1)];
			$assist[$num_calciatore] = $dati_voto[($ncs_assist-1)];
			$rigoriparati[$num_calciatore] = $dati_voto[($ncs_rigoreparato-1)];
			#malus
			$ammonizione[$num_calciatore] = $dati_voto[($ncs_ammonizione-1)];
			$espulsione[$num_calciatore] = $dati_voto[($ncs_espulsione-1)];
			$autogol[$num_calciatore] = $dati_voto[($ncs_autogol-1)];
			$rigorisbagliati[$num_calciatore] = $dati_voto[($ncs_rigoresbagliato-1)];
			$golsubiti[$num_calciatore] = $dati_voto[($ncs_golsubiti-1)];
			
			$presenza[$num_calciatore] = $dati_voto[($ncs_presenza-1)];
			$titolare[$num_calciatore] = $dati_voto[($ncs_titolare-1)];
				...
				if ($opermessi >= 0) {
					...
					for ($num2 = 0 ; $num2 < $num_linee_formazione; $num2++) {
						...
						if ($voto_tot[$num_calciatore] == 0 AND $voto_giornale[$num_calciatore] == 0) {
							...
							$$nome_sostituisci++;
							$num_sostituisci++;
							if( $entrato[$num_calciatore] == 1 || $titolare[$num_calciatore] == 1 ) {
								${$formazione}[$num2] = togli_acapo(${$formazione}[$num2]);
								${$formazione}[$num2] .= ",,".$voto_giornale[$num_calciatore]."\r\n";
							}
							...
							$golsegnati[$num_calciatore].",".$assist[$num_calciatore].",".$rigoriparati[$num_calciatore].",".
							$ammonizione[$num_calciatore].",".$espulsione[$num_calciatore].",".$rigorisbagliati[$num_calciatore].",".
							$autogol[$num_calciatore].",".$golsubiti[$num_calciatore]."\r\n";
					...
					if ($sostituisci_per_ruolo == "SI") {
						...
						for($num2 = $linea_inizio_panchina; $num2 < $num_linee_formazione; $num2++) {
							...
							if( ($voto_giornale[$num_calciatore] == 0 && $entrato[$num_calciatore] == 1 )
								|| ($voto_giornale[$num_calciatore] == 0 && $titolare[$num_calciatore] == 1 )
								) {
								${$formazione}[$num2] = togli_acapo(${$formazione}[$num2]);
								${$formazione}[$num2] .= ",,".$voto_giornale[$num_calciatore]."\r\n";									
							} 
			...
			*/
			...
			if ($tipo_campionato == "V") {
				...
				for($num1 = 1 ; $num1 < $linee; $num1++) {
					...
					$classifica[$outente][0] = $outente;
					$classifica[$outente][1] = $voti_tot[$outente];
					
				} # fine for $num1
				$nuovo_filegiornata .= "#@& fine punteggi #@&\r\n\r\n";
				
				### Calcolo classifica generale aggiornata e salvataggio su file
				$classifica = calcola_classifica($classifica, $num);
				salva_classifica($classifica, $num);
			...
			if ($tipo_campionato == "P") {
				...
				for($num1 = 1 ; $num1 < $linee; $num1++) {
					...
					$classifica[$outente][0] = $outente;
					$classifica[$outente][1] = $punti_tot[$outente];
			...
			if ($tipo_campionato == "S") {
				...
				for($num1 = 0 ; $num1 < $num_partite; $num1++) {
					...
					$voti_tot_fc[$partita[0]] = $voti_tot[$partita[0]] + $voti_bonus_in_casa;
					$voti_tot_fc[$partita[1]] = $voti_tot[$partita[1]];
					$gol1 = floor(($voti_tot_fc[$partita[0]] - $soglia_voti_primo_gol) / $incremento_voti_gol_successivi) + 1;

					if ($gol1 < 0) $gol1 = 0;

					$gol2 = floor(($voti_tot_fc[$partita[1]] - $soglia_voti_primo_gol) / $incremento_voti_gol_successivi) + 1;

					if ($gol2 < 0) $gol2 = 0;

					#criterio scarto di N punti nella stessa fascia
					if ($gol1 == $gol2 && $gol1 > 0 && $differenza_punti_a_parita_gol > 0){
						if($voti_tot_fc[$partita[0]] >= $voti_tot_fc[$partita[1]]+$differenza_punti_a_parita_gol) $gol1 = $gol1+1;
						else if($voti_tot_fc[$partita[1]] >= $voti_tot_fc[$partita[0]]+$differenza_punti_a_parita_gol) $gol2 = $gol2+1;
					}
					...
					if ($differenza_punti_prima_soglia_meno_sei > 0){
						if($voti_tot_fc[$partita[0]] < $otsoglia_voti_primo_gol-6 && $voti_tot_fc[$partita[1]] >= $otsoglia_voti_primo_gol-6
						&& $voti_tot_fc[$partita[1]] >= $voti_tot_fc[$partita[0]]+$differenza_punti_prima_soglia_meno_sei) $gol2 = $gol2+1;
						else if($voti_tot_fc[$partita[1]] < $otsoglia_voti_primo_gol-6 && $voti_tot_fc[$partita[0]] >= $otsoglia_voti_primo_gol-6
						&& $voti_tot_fc[$partita[0]] >= $voti_tot_fc[$partita[1]]+$differenza_punti_prima_soglia_meno_sei) $gol1 = $gol1+1;
					}
					#criterio differenza di N punti per gol premio
					if ($differenza_punti_gol_premio > 0){
					if($voti_tot_fc[$partita[0]] >= $voti_tot_fc[$partita[1]]+$differenza_punti_gol_premio) $gol1 = $gol1+1;
					else if($voti_tot_fc[$partita[1]] >= $voti_tot_fc[$partita[0]]+$differenza_punti_gol_premio) $gol2 = $gol2+1;
					}
					...
					if ($gol1 > $gol2) {
						...
						$vinte[$partita[0]] = 1; $vinte[$partita[1]] = 0;
						$nulle[$partita[0]] = 0; $nulle[$partita[1]] = 0;
						$perse[$partita[0]] = 0; $perse[$partita[1]] = 1;
						$vinte_casa[$partita[0]] = 1; $vinte_casa[$partita[1]] = 0;
						$nulle_casa[$partita[0]] = 0; $nulle_casa[$partita[1]] = 0;
						$perse_casa[$partita[0]] = 0; $perse_casa[$partita[1]] = 0;
						$vinte_fuori[$partita[0]] = 0; $vinte_fuori[$partita[1]] = 0;
						$nulle_fuori[$partita[0]] = 0; $nulle_fuori[$partita[1]] = 0;
						$perse_fuori[$partita[0]] = 0; $perse_fuori[$partita[1]] = 1;	
					} # fine if ($gol1 > $gol2)
					if ($gol1 == $gol2) {
						$punti_tot[$partita[0]] = $punti_partita_pareggiata;
						$punti_tot[$partita[1]] = $punti_partita_pareggiata;
						$vinte[$partita[0]] = 0; $vinte[$partita[1]] = 0;
						$nulle[$partita[0]] = 1; $nulle[$partita[1]] = 1;
						$perse[$partita[0]] = 0; $perse[$partita[1]] = 0;
						$vinte_casa[$partita[0]] = 0; $vinte_casa[$partita[1]] = 0;
						$nulle_casa[$partita[0]] = 1; $nulle_casa[$partita[1]] = 0;
						$perse_casa[$partita[0]] = 0; $perse_casa[$partita[1]] = 0;
						$vinte_fuori[$partita[0]] = 0; $vinte_fuori[$partita[1]] = 0;
						$nulle_fuori[$partita[0]] = 0; $nulle_fuori[$partita[1]] = 1;
						$perse_fuori[$partita[0]] = 0; $perse_fuori[$partita[1]] = 0;
					} # fine if ($gol1 == $gol2)
					if ($gol1 < $gol2) {
						$punti_tot[$partita[0]] = $punti_partita_persa;
						$punti_tot[$partita[1]] = $punti_partita_vinta;
						$vinte[$partita[0]] = 0; $vinte[$partita[1]] = 1;
						$nulle[$partita[0]] = 0; $nulle[$partita[1]] = 0;
						$perse[$partita[0]] = 1; $perse[$partita[1]] = 0;
						$vinte_casa[$partita[0]] = 0; $vinte_casa[$partita[1]] = 0;
						$nulle_casa[$partita[0]] = 0; $nulle_casa[$partita[1]] = 0;
						$perse_casa[$partita[0]] = 1; $perse_casa[$partita[1]] = 0;
						$vinte_fuori[$partita[0]] = 0; $vinte_fuori[$partita[1]] = 1;
						$nulle_fuori[$partita[0]] = 0; $nulle_fuori[$partita[1]] = 0;
						$perse_fuori[$partita[0]] = 0; $perse_fuori[$partita[1]] = 0;
					} # fine if ($gol1 < $gol2)
					
					$giocate[$partita[0]] = 1; $giocate[$partita[1]] = 1;
					$giocate_casa[$partita[0]] = 1; $giocate_casa[$partita[1]] = 0;
					$giocate_fuori[$partita[0]] = 0; $giocate_fuori[$partita[1]] = 1;
					$reti_fatte_casa[$partita[0]] = $gol1; $reti_fatte_casa[$partita[1]] = 0;
					$reti_subite_casa[$partita[0]] = $gol2; $reti_subite_casa[$partita[1]] = 0;
					$reti_fatte_fuori[$partita[0]] = 0; $reti_fatte_fuori[$partita[1]] = $gol2;
					$reti_subite_fuori[$partita[0]] = 0; $reti_subite_fuori[$partita[1]] = $gol1;
					$reti_fatte[$partita[0]] = $gol1; $reti_fatte[$partita[1]] = $gol2;
					$reti_subite[$partita[0]] = $gol2; $reti_subite[$partita[1]] = $gol1;
					$differenza_reti[$partita[0]] = $gol1-$gol2; $differenza_reti[$partita[1]] = $gol2-$gol1;
					$totale_punti_fatti[$partita[0]] = $voti_tot_fc[$partita[0]]; $totale_punti_fatti[$partita[1]] = $voti_tot_fc[$partita[1]];
				...
				for($num1 = 1 ; $num1 < $linee; $num1++) {
					@list($outente, $opass, $opermessi, $oemail, $ourl, $osquadra, $otorneo, $oserie, $ocittà, $ocrediti, $ovariazioni, $ocambi, $oreg) = explode("<del>", $file[$num1]);
					if ($opermessi >= 0) $nuovo_filegiornata .= $outente."##@@&&".$punti_tot[$outente]."##@@&&".$giocate[$outente]."##@@&&".$vinte[$outente].
					"##@@&&".$nulle[$outente]."##@@&&".$perse[$outente]."##@@&&".$giocate_casa[$outente]."##@@&&".$vinte_casa[$outente].
					"##@@&&".$nulle_casa[$outente]."##@@&&".$perse_casa[$outente]."##@@&&".$reti_fatte_casa[$outente]."##@@&&".$reti_subite_casa[$outente].
					"##@@&&".$giocate_fuori[$outente]."##@@&&".$vinte_fuori[$outente]."##@@&&".$nulle_fuori[$outente]."##@@&&".$perse_fuori[$outente].
					"##@@&&".$reti_fatte_fuori[$outente]."##@@&&".$reti_subite_fuori[$outente]."##@@&&".$reti_fatte[$outente]."##@@&&".$reti_subite[$outente].
					"##@@&&".$differenza_reti[$outente]."##@@&&".$totale_punti_fatti[$outente]."\r\n";
					
					$classifica[$outente][0] = $outente;
					$classifica[$outente][1] = $punti_tot[$outente]; $classifica[$outente][2] = $giocate[$outente];
					$classifica[$outente][3] = $vinte[$outente]; $classifica[$outente][4] = $nulle[$outente];
					$classifica[$outente][5] = $perse[$outente]; $classifica[$outente][6] = $giocate_casa[$outente];
					$classifica[$outente][7] = $vinte_casa[$outente]; $classifica[$outente][8] = $nulle_casa[$outente];
					$classifica[$outente][9] = $perse_casa[$outente]; $classifica[$outente][10] = $reti_fatte_casa[$outente];
					$classifica[$outente][11] = $reti_subite_casa[$outente]; $classifica[$outente][12] = $giocate_fuori[$outente];
					$classifica[$outente][13] = $vinte_fuori[$outente]; $classifica[$outente][14] = $nulle_fuori[$outente];
					$classifica[$outente][15] = $perse_fuori[$outente]; $classifica[$outente][16] = $reti_fatte_fuori[$outente];
					$classifica[$outente][17] = $reti_subite_fuori[$outente]; $classifica[$outente][18] = $reti_fatte[$outente];
					$classifica[$outente][19] = $reti_subite[$outente]; $classifica[$outente][20] = $differenza_reti[$outente];
					$classifica[$outente][21] = $totale_punti_fatti[$outente];
				} # fine for $num1
				$nuovo_filegiornata .= "#@& fine punteggi #@&\r\n\r\n";
