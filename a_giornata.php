...
if ($_SESSION['valido'] == "SI" and $_SESSION['permessi'] >= 4) {
	...
	if ($cancella_giornata == "SI"){
		...
		#Ricalcolo classifica dopo eliminazione giornata
		$classifica = calcola_classifica(null, $n_torneo);
		if(count($classifica) == 0){
			$fileCl=$percorso_cartella_dati."/classifica_".$n_torneo;
			if (!unlink($fileCl)) {
				echo "Cancellazione di $fileCl non riuscita ...\n";
			}
		}
		else{
			#Salvataggio su file...
			salva_classifica($classifica, $n_torneo);
		}
