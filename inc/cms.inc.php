...
function pagina($paginaid) {
	...
	if ($dati_pagine["pattivo"] != "NO" ) {
		...
		echo "<div class='box_utente_header'>"
		.html_entity_decode(aggiusta_tag($dati_pagine["ptitolo"]))
		."</div><div class='box_utente_content'>
...
function categoria($categoria) {
	...
	foreach($dati_categorie as $chiave => $valore) {
		...
		if ($categoria == $valore["ptitolo"]) {
			echo "<div class='box_utente_header'>" .html_entity_decode($valore["ptitolo"]). "</div>"
			...
			echo link_notizie($categoria)."<br /><br /><p class='date'> |&nbsp;".mostra_data($data_vis)."&nbsp;&nbsp;|&nbsp;".$valore["putente"]."&nbsp;&nbsp;&nbsp;".$adm_opz."</p></div>";
			$trovato = "SI";
			break;
		}
	}
...
function notizia($notiziaid, $evidenzia) {
	...
	if ($dati_notizie["pattivo"] != "NO" ) {
		...
		echo "<div class='articolo_sh'>".html_entity_decode($dati_notizie["ptitolo"])
		."<div style='float:right;display:inline-block'><img src='./immagini/puntina.png' width='25px'></div>
		<div style='clear:both'></div></div>".$acapo;
		echo "<div class='articolo_s'>";
		if($dati_notizie["ptesto"]){
			echo "".html_entity_decode($dati_notizie["ptesto"]).$acapo;
		}
		else
...
function modifica_notizia() {
	...
	if ($conferma == "SI"){
		...
		$V_dati_notizie["ptitolo"]		= $dati_notizie["ptitolo"];
		$V_dati_notizie["pabstract"]		= $dati_notizie["pabstract"];
		$V_dati_notizie["ptesto"]		= $dati_notizie["ptesto"];
		$V_dati_notizie["pbox"]			= $dati_notizie["pbox"];
		...
		<br />
		<textarea id='pagetext' name='pabstract' rows='20' cols='120'>".$V_dati_notizie["pabstract"]."</textarea>
		<br />
		<b>Contenuto notizia:</b><br/>
		<br />
