...
if ($_SESSION['valido'] == "SI" and $_SESSION['permessi'] == 5) {
	...
	if ($inserimento == "scrivi") {
		...
		elseif ($azione == "modifica") {
			...
			#Ricalcolo classifica generale e salvataggio su file
			$classifica = calcola_classifica(null, $N_otid);
			salva_classifica($classifica, $N_otid);
			#Fine Ricalcolo
		...
		if(!$itorneo) {
			if (!@is_file($percorso_cartella_dati."/tornei.php")) {
				...
				fclose($fp) OR die ("errore fileclose");
					if (@chmod ($fp, 0664)) echo "CHMOD 664 impostato!<br />";
				unset ($ini_file,$fp);
			}
			...
			if ($azione == "nuovo"){
				...
				$tornei = @file($percorso_cartella_dati."/tornei.php");
				list($otid, $otdenom, $otpart, $otserie, $otmercato_libero, $ottipo_calcolo, $otgiornate_totali, $otritardo_torneo, $otcrediti_iniziali, $otnumcalciatori, $otcomposizione_squadra, $ottemp1, $temp2, $temp3, $temp4, $otstato, $otmodificatore_difesa, $otschemi, $otmax_in_panchina, $otpanchina_fissa, $otmax_entrate_dalla_panchina, $otsostituisci_per_ruolo, $otsostituisci_per_schema,  $otsostituisci_fantasisti_come_centrocampisti, $otnumero_cambi_max, $otrip_cambi_numero, $otrip_cambi_giornate, $otrip_cambi_durata, $otaspetta_giorni, $otaspetta_ore, $otaspetta_minuti, $otnum_calciatori_scambiabili, $otscambio_con_soldi, $otvendi_costo, $otpercentuale_vendita, $otsoglia_voti_primo_gol, $otincremento_voti_gol_successivi, $otvoti_bonus_in_casa, $otpunti_partita_vinta, $otpunti_partita_pareggiata, $otpunti_partita_persa, $otdifferenza_punti_a_parita_gol, $otdifferenza_punti_zero_a_zero, $otdifferenza_punti_prima_soglia_sei, $otdifferenza_punti_gol_premio, $otmin_num_titolari_in_formazione, $otpunti_pareggio, $otpunti_pos, $otreset_scadenza) = explode(",", $tornei[$itorneo]);
			}
			...
			<tr>
			<td>ID torneo</td>
			<td align="center"><input type="hidden" name="N_otid" value="<?php echo $otid ?>" /><?php echo "$otid"; ?></td>
			<td align="left">Progressivo ad uso interno non modificabile </td>
			</tr>
			<tr>
			<td>Denominazione</td>
			<td align="center"><input type="text" name="N_otdenom" value="<?php echo $otdenom?>" size="50" maxlength="50" /></td>
			...
			- "R" riparazione (fase post-asta in cui si rettificano e completano le squadre - <b>solo $mercato_libero = "NO"</b>). <br />
			- "B" buste chiuse (permette di fare offerte nascoste - <b>solo $mercato_libero = "NO"</b>).</td>
			</tr>
			<tr>
			<td>Partecipanti</td>
			<td align="center"><input type="text" name="N_otpart" value="<?php echo $otpart?>" size="4" maxlength="4" /></td>
			...
			<option value="SI" <?php if($otmercato_libero == "SI") echo "selected"; ?> > Mercato libero</option>
			<option value="NO" <?php if($otmercato_libero == "NO") echo "selected"; ?> > Asta iniziale</option>
			</select></td>
			...
			<td align="center">
			<input name="N_otgiornate_totali" type="text" size="3" maxlength="2" value="<?php echo $otgiornate_totali?>" /></td>
			<td align="left">Il numero di giornate complessive del campionato. </td>
			</tr>
			<tr>
			<td>Ritardo inizio torneo</td>
			<td align="center"><input name="N_otritardo_torneo" type="text" value="<?php echo $otritardo_torneo?>" size="3" maxlength="2" /></td>
			...
			<td align="center"><input name="N_otnumcalciatori" type="text" value="<?php echo $otnumcalciatori?>" size="3" maxlength="4" /></td>
			<td align="left">Il totale dei calciatori acquistabili. </td>
			</tr>
			<tr>
			<td>Composizione squadra </td>
			<td align="center"><input name="N_otcomposizione_squadra" type="text" value="<?php echo $otcomposizione_squadra?>" size="6" maxlength="5" /></td>
			<td align="left">Esempi: &quot;38806&quot;,&quot;38725&quot;,&quot;38815&quot;,&quot;38716&quot;. La somma deve essere uguale al numero dei calciatori previsti per questo campionato.</td>
			</tr>
			<tr>
			...
			<td align="center"><input type="text" name="N_otschemi" value="<?php echo $otschemi ?>" size="40" maxlength="100" /></td>
			<td align="left">Gli schemi di gioco utilizzabili. Gli schemi a 5 numeri servono solo se si usano i fantasisti. Si possono aggiungere o togliere schemi. <br />IMPORTANTE: separare gli schemi con un trattino: <br />1343-1352-1451-1442-1433-1541-1532 - 13403-13502-14501-14402-14303-15401-15302</td>
			</tr>
			<tr>
			<td>Numero panchinari</td>
			<td align="center"><input name="N_otmax_in_panchina" type="text" value="<?php echo $otmax_in_panchina?>" size="3" maxlength="2" /></td>
			...
			<td align="left">impostare a "SI" per avere la panchina (1222 come PDCA) altrimenti "NO" per la panchina libera</td>
			</tr>
			<tr>
			<td>Numero panchinari che possono entrare per sostituzione</td>
			<td align="center"><input name="N_otmax_entrate_dalla_panchina" type="text" value="<?php echo $otmax_entrate_dalla_panchina?>" size="3" maxlength="2" /></td>
			<td align="left">Numero di calciatori in panchina che possono essere utilizzati per sostituire i titolari. Si possono fare sostituzioni per ruolo (il calciatore entra se un'altro del suo ruolo non ha giocato) o per schema (il calciatore entra se entrando lo schema che si forma &egrave; tra quelli consentiti). Se sia per ruolo che per schema sono a SI si sostituisce prima per ruolo.</td>
			</tr>
			<tr>
			...
			<td align="left">Nel caso in cui un calciatore titolare non prenda voto la sostituzione con il panchinaro avviene per ruolo. <b>Selezionare almeno una tra le opzioni di sostituzione (<u>per ruolo e per schema</u>), altrimenti non entreranno panchinari in caso di possibile sostituzione</b>.</td>
			</tr>
			<tr>
			<td>Panchina sostituisci per schema</td>
			<td align="center">
			<select name="N_otsostituisci_per_schema">
			<option value="SI" <?php if($otsostituisci_per_schema == "SI") echo "selected"; ?> > SI</option>
			<option value="NO" <?php if($otsostituisci_per_schema == "NO") echo "selected"; ?> > NO</option>
			</select></td>
			<td align="left">Qualora la sostituzione per ruolo non sia insufficiente a completare la rosa effettua la sostituzione per schema. <b>Selezionare almeno una tra le opzioni di sostituzione (<u>per ruolo e per schema</u>), altrimenti non entreranno panchinari in caso di possibile sostituzione</b>.</td>
			</tr>
			<tr>
			<td>Sostituisci fantasisti come centrocampisti</td>
			<td align="center">
			<select name="N_otsostituisci_fantasisti_come_centrocampisti">
			<option value="SI" <?php if($otsostituisci_fantasisti_come_centrocampisti == "SI") echo "selected"; ?> > SI</option>
			<option value="NO" <?php if($otsostituisci_fantasisti_come_centrocampisti == "NO") echo "selected"; ?> > NO</option>
			</select></td>
			<td align="left">E' una impostazione utilizzata qualche anno fa; impostare a "SI", usato solo con sostituzioni per ruolo, massimo 1 fantasista. Attualmente non influente.</td>
			</tr>
			<tr>
			<td>Messaggeria</td>
			<td align="center">
			...
			if ($otmercato_libero == "SI") {
				...
				<option value='0'"; if($otrip_cambi_durata == "0") echo "selected"; echo " > Una giornata</option>
				<option value='1'"; if($otrip_cambi_durata == "1") echo "selected"; echo " > Due giornate</option>
				</select></td>
			...
			if ($ottipo_calcolo == "P" OR $ottipo_calcolo == "S") {
				...
				<td align='center'><input name='N_otsoglia_voti_primo_gol' id='N_otsoglia_voti_primo_gol' type='text' value='$otsoglia_voti_primo_gol' size='3' maxlength='2' 
				onkeyup=\"
				document.getElementById('label_diff_prima_soglia_meno_sei').innerHTML = (this.value - 6);
				document.getElementById('desc_diff_prima_soglia_meno_sei').innerHTML = (this.value - 6);
				document.getElementById('desc_diff_prima_soglia').innerHTML = (this.value);
				\"/></td>
				<td align='left'>&nbsp;</td>
				</tr>
				<tr>
				...
				<tr>
				<td>Distacco punti a parità di gol</td>
				...
				<td align='left'>Se due squadre avversarie totalizzano entrambe un punteggio all'interno della stessa fascia,
  				si aggiunge un gol alla squadra che avrà distaccato l'avversario di almeno
  				tanti punti quanti specificati da questo parametro. Questa opzione serve a LIMITARE I PAREGGI.
  				<b>0 disabilita l'opzione</b>.</td>
				</tr>
				<tr>
				<td>Distacco punti in caso di 0 - 0</td>
				...
				<td align='left'>Se due squadre avversarie totalizzano entrambe un punteggio all'interno della prima fascia (0 - 0),
  				si aggiunge un gol alla squadra che avrà distaccato l'avversario di almeno
  				tanti punti quanti specificati da questo parametro. Questa opzione serve a LIMITARE I PAREGGI sotto la prima soglia.
  				<b>0 disabilita l'opzione</b>.</td>
				</tr>
				<tr>
				<td>Distacco punti per penalizzazione, se meno di <span id='label_diff_prima_soglia_meno_sei'>60</span> punti</td>
				...
				<option value='M'"; if($otpunti_pareggio == "M"); echo " > Media</option>
				<option value='A'"; if($otpunti_pareggio == "A") echo "selected"; echo " > Alta</option>
				<option value='B'"; if($otpunti_pareggio == "B"); echo " > Bassa</option>
				</select></td>
				...
				document.getElementById('label_diff_prima_soglia_meno_sei').innerHTML = (document.getElementById('N_otsoglia_voti_primo_gol').value - 6);
				document.getElementById('desc_diff_prima_soglia_meno_sei').innerHTML = (document.getElementById('N_otsoglia_voti_primo_gol').value - 6);
				document.getElementById('desc_diff_prima_soglia').innerHTML = (document.getElementById('N_otsoglia_voti_primo_gol').value);
  				</script>
				";
			}
