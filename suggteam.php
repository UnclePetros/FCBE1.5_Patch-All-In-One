...
if(!$modulo) {$dif=3;$cen=4;$att=3;}
else{
	@list($dif,$cen,$att) = explode("-", $modulo);
}
...
echo "<div class='box_utente_header-noround2'>MIGLIOR FORMAZIONE SQUADRA</div>
<div class='box_utente_content_simple'>";
$top11_table = "<table width='100%' cellpadding='1' cellspacing='2' align='center'>
...
$top11_table_pan = "<table width='100%' cellpadding='1' cellspacing='2' align='center'>
<tr><td class='testa'>Num.</td>
<td class='testa'>Nome</td>
<td class='testa'>Ruolo</td>
<td class='testa'>Partite</td>
...
$partite_giocate_tot = 0;
$partite_giocate_giornale = 0;
$somma_voti_tot = 0;
...
if ($partite_giocate_tot != 0) {
$media_giornale = round(($somma_voti_giornale /$partite_giocate_giornale),2);
$media_punti = round(($somma_voti_tot / $partite_giocate_tot),2);
...
if ($ultima_giornata >= 1) $calciatori = file("$percorso_cartella_voti/voti$ultima_giornata.txt");
...
$portieri[]=array($nome,$totpresenze,$media_giornale,$tot_golsegnati,$totamm,$totesp,$media_punti+10,$totass,$numero);
}
if ($ruolo == "D" and $totpresenze >= $partite_minime_per_range){
$difensori[]=array($nome,$totpresenze,$media_giornale,$tot_golsegnati,$totamm,$totesp,$media_punti+10,$totass,$numero);
}

if ($ruolo == "C" and $totpresenze >= $partite_minime_per_range){
$centrocampisti[]=array($nome,$totpresenze,$media_giornale,$tot_golsegnati,$totamm,$totesp,$media_punti+10,$totass,$numero);
}

if ($ruolo == "A" and $totpresenze >= $partite_minime_per_range){
$attaccanti[]=array($nome,$totpresenze,$media_giornale,$tot_golsegnati,$totamm,$totesp,$media_punti+10,$totass,$numero);
}

if ($ruolo == "F" and $totpresenze >= $partite_minime_per_range){
$fantasisti[]=array($nome,$totpresenze,$media_giornale,$tot_golsegnati,$totamm,$totesp,$media_punti+10,$totass,$numero);
}
...
$top11_table .= "<tr bgcolor = '$color'>";
$top11_table .= "<td align='center'><a href='stat_calciatore.php?num_calciatore=$numero&ruolo_guarda=$ruolo_guarda' class='user'>$numero</a></td>
...
<td align='center'>$stat_ammonizioni</td>
<td align='center'>$stat_espulsione</td>
</tr>";
$top11_str = "".$numero;

$nome = $portieri[1][0];
$totpresenze = $portieri[1][1];
$media_giornale = $portieri[1][2];
$tot_golsegnati = $portieri[1][3];
$stat_ammonizioni = $portieri[1][4];
$stat_espulsione = $portieri[1][5];
$media_punti = $portieri[1][6]-10;
$totass = $portieri[1][7];
$numero = $portieri[1][8];

$top11_table_pan .= "<tr bgcolor = '$color'>";
$top11_table_pan .= "<td align='center'><a href='stat_calciatore.php?num_calciatore=$numero&ruolo_guarda=$ruolo_guarda' class='user'>$numero</a></td>
<td align='left'>$nome</td>
<td align='center'>P</td>
<td align='center'>$totpresenze</td>
<td align='center'>$media_giornale</td>
...
$top11_table .= "</table>";
$top11_table_pan .= "</table>";

echo "<form method='post' action='./suggteam.php'>
Seleziona il modulo: <select name='modulo'>
...
<div align='center' style='margin: 10px 0px 10px 0px'><img src='./fantacampo.php?orientamento_campetto=1&riduci=75&id_gioc=$top11_str' alt='La tua squadra in campo' /></div>
<div>$top11_table</div>
<div style='margin-top: 10px'>$top11_table_pan</div>
</div>";
}
else{
	echo "<div class='box-shadow' style='height:90px;line-height:90px'><span style='font-weight:bold'>
Funzionalità non disponibile. E' necessario che sia stata giocata almeno una giornata di campionato.
</span></div>";
}
